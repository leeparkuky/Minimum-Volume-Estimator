class MVE:
    ...
    def gen_md_sq(self):
        ...
        MD_sq = np.square(list(map(lambda i:  dist.pairwise([self.X.iloc[i,:], X_bar])[1,0]   ,   range(self.n))))
    ...
    def gen_mj_sq(self):
        ...
        dist = DistanceMetric.get_metric('mahalanobis', V =  cov)
        MD_sq = np.square(list(map(lambda i:  dist.pairwise([self.X.iloc[i,:], self.xj])[1,0]   ,   range(self.n))))
        MD_sq.sort()
        md_sq = MD_sq[self.h]
        self.md_sq = md_sq
